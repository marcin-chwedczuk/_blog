...
must put extra effort to ensure that tests fail
...
enough time or simply just don't bother to check if their tests fail with
...
we will get an error message like this (formatting added by me):
...
It contains informations that are easily obtainable by looking at the
test method's source code, but it does not contain
the most important piece of the information: which properties 
...
{% endhighlight %}
Now the error message looks like (formatting added by me):
{% highlight nohighlight %}
Expected to receive a call matching:
    DoStuff(e => ((((e.Id == 9) AndAlso 
        (e.FirstName == "jan")) AndAlso 
...
then I would prefer to look for a better solution that is more automatic.

There is also another problem that we ignored so far.
...
In order to ensure that our matchers remain valid,
we must go through all argument matchers 
for `StringableArgument` class and make sure that they use the new field.

Above problem may be solved by moving equality checking
...
// in a test method:
_fooService.Received()
    // NOTICE: We no longer use lambda expression.
...
We also get a slightly better error message (with some formatting added):
{% highlight nohighlight %}
Expected to receive a call matching:
    DoStuff(StringableArgument(
        id: 9, firstName: "jan", 
...
{% endhighlight %}
The error message that we got is:
{% highlight nohighlight %}
...
unexpected values. Some messages seem a little bit too verbose for me
...
As we can see there exists no perfect solution. Still by a bit of effort
